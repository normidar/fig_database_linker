...
class DataLinkerPostgres extends DataLinkerAbs {
  ...
  Future<List<String>> getTables() async {
    List<List<dynamic>> results =
        await _conn?.query('select tablename from pg_tables');
    List<String> rt = [];
  ...
  Future<List<List<String>>> getTableView(String table,
    ...
    return await getRows("SELECT * FROM $table ORDER BY $idName" +
        (desc ? ' DESC' : '') +
        " LIMIT " +
        count.toString());
  }
  ...
  ///实现
  @override
  Future<String> deleteDataById(String table, String id,
      {String idName = ''}) async {
  ...
  @override
  Future<String> updataDataById(String table, Map<String, dynamic> data,
      {String idName = ''}) async {
    if (idName.isEmpty) idName = await getIdName(table);
    var setString = '';
    //循环合成SET的成员
